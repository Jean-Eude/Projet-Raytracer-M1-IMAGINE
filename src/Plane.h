...
public:
    ...
    bool isParallelTo( Line const & L ) const {
        //TODO completer
        return Vec3::dot(L.direction(), m_normal);
    }
    Vec3 getIntersectionPoint( Line const & L ) const {
        ...
        if(!isParallelTo(L)) {
            float t = (Vec3::dot(this->m_center, this->m_normal) - Vec3::dot(L.origin(), this->m_normal)) / Vec3::dot(L.direction(), this->m_normal);
            Vec3 inter = L.origin() + t * L.direction();
            result = inter;
        }
