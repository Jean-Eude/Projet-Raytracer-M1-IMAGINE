...
public:
    ...
    RayTriangleIntersection intersect( Ray const & ray ) const {
        ...
        // Parcours des faces et création d'un triangle / face  + scale du triangle créé
        for(int i = 0; i < triangles.size(); i++) {  
            Triangle triangle = Triangle(vertices[triangles[i].v[0]].position * triangleScaling, vertices[triangles[i].v[1]].position * triangleScaling, vertices[triangles[i].v[2]].position * triangleScaling);  
            
            // Récupération de l'intersection dans un nouveau RayTriangleIntersection
            RayTriangleIntersection inter = triangle.getIntersection(ray);

            // Si la condition est respectée, j'attribue les valeurs récupérées avec la fonction d'intersection du triangle à closestIntersection
            if(inter.intersectionExists && inter.t < closestIntersection.t) {
                closestIntersection.t = inter.t;
                closestIntersection.intersectionExists = true;
                closestIntersection.w0 = inter.w0;
                closestIntersection.w1 = inter.w1;
                closestIntersection.w2 = inter.w2;
                closestIntersection.tIndex = i;

                Vec3 normale = closestIntersection.w0 * vertices[triangles[i].v[0]].normal + closestIntersection.w1 * vertices[triangles[i].v[1]].normal + closestIntersection.w2 * vertices[triangles[i].v[2]].normal;
                normale.normalize();
                closestIntersection.normal = normale;
            }
        }
