...
public:
    ...
    RaySphereIntersection intersect(const Ray &ray) const {
        ...
        // Calcul des différents éléments pour calculer le discriminant puis t
        Vec3 OC = ray.origin() - m_center;
        float DotD = Vec3::dot(ray.direction(), ray.direction());

        float a = DotD;
        float b = 2 * Vec3::dot(ray.direction(), OC);
        ...
        if(discri < 0.) {
            ...
            // Calcul de la racine la plus petite
            ...
            // Calcul de la racine la plus grande
            ...
            // On regarde qui est la plus petite et la plus grande des 2 racines
            tmin = std::min(t1, t2);
            tmax = std::max(t1, t2);
            ...
            // 1ère intersection (la plus proche)
            intersection.intersection = ray.origin() + tmin * ray.direction();
            
            // 2ème intersection (la plus éloignée)
            intersection.secondintersection = ray.origin() + tmax * ray.direction();

            // Calcul de la normale
            Vec3 n = intersection.intersection - m_center;
            n.normalize();
            intersection.normal = n;

            // Calcul des coordonées de textures pour une sphère
            float theta = acos(-intersection.normal[1]);
            float phi = atan2(-intersection.normal[2], intersection.normal[0]);

            intersection.u = 1 - (phi + M_PI) / (2 * M_PI); // Normalisation [0,1]
            intersection.v = (theta + M_PI / 2) / M_PI;
        }
